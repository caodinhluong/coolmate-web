import { getAllPromotions, getAllPromotionsWithoutPagination, getPromotionsById, createPromotions, updatePromotions, deletePromotions } from '../services/promotions.service.js';
...
export const getAllWithoutPagination = async (req, res) => {
	try {
		const data = await getAllPromotionsWithoutPagination();
		res.status(200).json({ data });
	} catch (error) {
		logger.error(`Get all promotions without pagination failed: ${error.message}`, { stack: error.stack });
		if (error.message === 'Database error: ER_NO_SUCH_TABLE') {
			res.status(400).json({ message: 'Table promotions does not exist' });
		} else if (error.message.includes('Database error')) {
			res.status(500).json({ message: 'Database error occurred' });
