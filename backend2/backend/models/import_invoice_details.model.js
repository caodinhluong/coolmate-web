...
class Import_invoice_details {
	...
	static async getAllWithoutPagination() {
		const sql = 'SELECT * FROM import_invoice_details';
	...
	static async insert(import_invoice_details) {
		...
		const [result] = await this.query(sql, [import_invoice_details]);
		return result;
	}

	static async update(id, import_invoice_details) {
		const sql = 'UPDATE import_invoice_details SET ? WHERE detail_id = ?';
		const [result] = await this.query(sql, [import_invoice_details, id]);
		return result;
	}
